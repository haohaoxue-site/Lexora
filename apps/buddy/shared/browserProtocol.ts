...
export const BROWSER_WAIT_CONDITIONS = ['page-ready', 'url-changed'] as const
...
export const browserActionSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    kind: z.literal('wait'),
    timeoutMs: z.number().int().min(1).max(BROWSER_MAX_WAIT_TIMEOUT_MS),
  }).strict(),
])
...
export const browserStateSnapshotSchema = z.object({
  ...
  pageId: browserIdSchema,
  profileMode: z.enum(['ephemeral', 'personal']),
...
export const browserActResultSchema = z.discriminatedUnion('ok', [
  z.object({
    ...
    ok: z.literal(true),
    requiresObservation: z.literal(true),
    state: browserStateSnapshotSchema,
  }).strict(),
  browserFailureResultSchema,
])
