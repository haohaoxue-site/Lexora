...
import type { LocalChangeSetDetail } from '@buddy-shared/changes/changeApi'
...
const tabsScrollbar = useTemplateRef<ScrollbarInst>('tabsScrollbar')
...
watch(() => props.tabs.map(tab => tab.id), (ids) => {
  changeFileLabels.value = Object.fromEntries(Object.entries(changeFileLabels.value).filter(([id]) => ids.includes(id)))
})
...
function handleTabsWheel(event: WheelEvent) {
  ...
  const multiplier = event.deltaMode === WheelEvent.DOM_DELTA_LINE
    ...
    : event.deltaMode === WheelEvent.DOM_DELTA_PAGE
      ? tabsScrollRoot.value?.clientWidth ?? 1
      : 1
  tabsScrollbar.value?.scrollBy({ left: event.deltaY * multiplier, behavior: 'auto' })
}
</script>
      ...
      <NEmpty :description="t('desktop.context.empty')">
        <template #extra>
          <NButton data-testid="task-context-open-browser" @click="emit('openBrowser')">
            <template #icon>
              <DesktopIcon :component="Globe16Regular" />
            </template>
