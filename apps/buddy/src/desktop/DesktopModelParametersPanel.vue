...
async function save() {
  ...
  if (props.model.source === 'manual' && !props.model.hasParameterOverride) {
    ...
    succeeded = await chat.upsertManualModel(props.model.providerId, input)
  }
  else {
    succeeded = await chat.setModelParameters(props.model.providerId, props.model.modelId, {
      contextWindow: form.contextWindow,
      maxTokens: form.maxTokens,
    })
  }
...
async function restoreDefaults() {
  if (await chat.restoreModelSourceParameters(props.model.providerId, props.model.modelId))
    editing.value = false
}

async function keepCustomParameters() {
  await chat.acknowledgeModelSourceUpdate(props.model.providerId, props.model.modelId)
}
</script>
