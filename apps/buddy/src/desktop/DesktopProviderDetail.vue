...
import DesktopModelDetailDialog from './DesktopModelDetailDialog.vue'
import { desktopProviderApiOptions } from './desktopProviderApiOptions'
import DesktopProviderConnectionDialog from './DesktopProviderConnectionDialog.vue'
...
const provider = computed(() => chat.providers.value.find(item => item.id === props.providerId) ?? null)
const models = computed(() => chat.registeredModels.value.filter(
...
async function saveManualModel(model: LocalCustomProviderModel) {
  ...
  if (await chat.upsertManualModel(props.providerId, model)) {
...
function formatTokens(value: number): string {
  return new Intl.NumberFormat(chat.language.value).format(value)
}

async function removeProvider() {
  if (await chat.removeProvider(props.providerId))
    emit('back')
}
</script>
...
<template>
  <div v-if="provider" class="desktop-provider-detail">
    <NAlert v-if="chat.agentError.value && !showConnectionDialog" type="error" :show-icon="false">
        ...
        <div class="desktop-provider-detail__row">
          ...
          <NSpace v-if="!provider.storedCredentialType">
            <NButton
              ...
              @click="chat.loginProvider(provider.id, authType)"
            >
          ...
          <NPopconfirm
            ...
            @positive-click="chat.clearProviderCredential(provider.id)"
          >
            <template #trigger>
            ...
            <template #trigger>
              <span>
                <NButton
                  ...
                  :loading="chat.syncingProviderId.value === provider.id"
                  @click="chat.syncProviderModels(provider.id)"
                >
      ...
      <div class="desktop-provider-detail__group">
        ...
        >
          ...
          <NSwitch
            ...
            @click.stop
            @update:value="chat.setProviderModelEnabled(provider.id, model.modelId, $event)"
          />
        </div>
      </div>
    </section>
