...
import type { LocalRun, LocalRunEvent } from '../../electron/shared/localChatApi'
...
watch(selectedRunId, async (runId) => {
  try {
    events.value = runId ? await chat.listRunEvents(runId) : []
  }
  catch {
    events.value = []
...
async function loadRuns() {
  ...
  try {
    runs.value = await chat.listRecentRuns()
...
function formatDate(value: string | null) {
  ...
  return Number.isNaN(date.getTime())
    ? value
    : new Intl.DateTimeFormat(chat.language.value, { dateStyle: 'short', timeStyle: 'medium' }).format(date)
}
