...
pub(super) fn native_pet_runtime_resolve_restored_placement(
    saved_position: NativePetPosition,
    window_size: NativePetLogicalSize,
) -> NativePetWindowPlacement {
    let Some(layout) = NativePetMonitorLayout::capture() else {
        return NativePetWindowPlacement {
            layer_shell_position: saved_position,
            monitor_index: None,
            position: saved_position,
        };
    };

    native_pet_resolve_restored_placement(
        &layout,
        saved_position,
        window_size,
        &NativePetBoundsPolicy::default(),
...
pub(super) fn native_pet_resolve_restored_placement(
    layout: &NativePetMonitorLayout,
    saved_position: NativePetPosition,
    window_size: NativePetLogicalSize,
    policy: &NativePetBoundsPolicy,
) -> NativePetWindowPlacement {
    let saved_rect = native_pet_window_rect(saved_position, window_size);
    if layout.monitor_at_point(saved_rect.center()).is_none() {
        return native_pet_initial_placement(layout, window_size, policy);
    }

    native_pet_resolve_window_placement(layout, saved_position, window_size, None, policy).placement
}
