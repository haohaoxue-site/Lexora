...
use crate::native_pet::{
    animation::{
        ...
        NativePetLifecycleAnimationDecision, NativePetManifest, NativePetRequestedAnimationState,
    },
...
fn manifest_only_finite_scripted_playback() -> (NativePetAnimationSet, NativePetAnimationPlayback) {
...
fn uses_hover_animation_only_when_no_higher_priority_animation_is_active() {
    ...
    assert_eq!(
        native_pet_animation_for_hover_state(true, false, false, requested_idle_state(), "idle",)
            .animation_target(),
        targets.hover()
    );
    assert_eq!(
        native_pet_animation_for_hover_state(false, false, false, requested_idle_state(), "hover",)
            .animation_target(),
        targets.idle()
    );
    assert_eq!(
...
fn hover_does_not_dilute_task_and_status_feedback() {
    ...
    assert_eq!(
        native_pet_animation_for_hover_state(true, false, false, requested_state("sad"), "sad",)
            .animation_target(),
        test_target("sad")
    );
}
...
fn hover_wakes_sleeping_pet_without_skipping_the_wake_transition() {
    ...
    assert_eq!(
        native_pet_animation_for_hover_state(true, false, false, requested_idle_state(), "sleep",)
            .animation_target(),
        targets.wake()
    );
    assert_eq!(
        native_pet_animation_for_hover_state(false, false, false, requested_idle_state(), "sleep",)
            .animation_target(),
        targets.sleep()
    );
}
...
fn lifecycle_preserves_manifest_only_requested_animation_target() {
    ...
    let animations =
        NativePetAnimationSet::from_manifest(manifest).expect("manifest with future loop loads");
