...
import { BuddySessionStorageError } from '../BuddySessionErrors'
...
describe('buddyAgentRunner', () => {
  it('bounds automation background session startup and records a stable timeout', async () => {
    ...
    const runner = fixture.createRunnerFromFactory(
      ...
      new BuddySessionRegistry<BuddyAgentSessionLike>(),
  ...
  it('does not prompt a session that finishes initializing after cancellation', async () => {
    ...
    const session: BuddyAgentSessionLike = {
      abort: async () => {},
      abortCompaction: () => {},
      activateTurn: async () => () => {},
      canCompact: () => true,
...
class OfflineSession implements BuddyAgentSessionLike {
...
class ReasoningFailureSession implements BuddyAgentSessionLike {
  readonly #listeners = new Set<(event: AgentSessionEvent) => void>()
...
class WaitingSession implements BuddyAgentSessionLike {
  readonly #listeners = new Set<(event: AgentSessionEvent) => void>()
...
class ModelFailureSession implements BuddyAgentSessionLike {
  readonly #listeners = new Set<(event: AgentSessionEvent) => void>()
...
class IdleSession implements BuddyAgentSessionLike {
  activateTurn: BuddyAgentSessionLike['activateTurn'] = async () => () => {}
...
async function createFixture(options: Pick<RunEventLogCallbacks, 'onEvent'> = {}) {
  ...
  return {
    ...
    createRunner(session: BuddyAgentSessionLike) {
    ...
    createRunnerFromFactory(
      ...
      sessions = new BuddySessionRegistry<BuddyAgentSessionLike>(),
