...
describe('browser session composition', () => {
  it.each(['interactive', 'automation_background'] as const)('keeps automation availability tied to session mode: %s', async (sessionMode) => {
    ...
    const names = composition.inProcessExtensions.map(extension => extension.name)
  ...
  it('validates local HTML against the session grant before authorization', async () => {
    ...
    const composition = await createComposition(root, requestApproval)
    const policy = activateToolPolicy(composition)
...
async function createComposition(
  ...
  return createBuddySessionComposition({
    canonicalRoot: root,
    conversationId: 'conversation-1',
...
function activateToolPolicy(
  composition: Awaited<ReturnType<typeof createBuddySessionComposition>>,
) {
  let handler: ((event: unknown) => Promise<unknown>) | null = null
  const onAuthorized = vi.fn(async () => {})
  composition.runContext.current = {
    flushProjectedEvents: async () => {},
    onToolExecutionAuthorized: onAuthorized,
    runId: 'run-1',
    signal: new AbortController().signal,
  }
  const extension = composition.inProcessExtensions.find(
...
function createCompositionServices(
...
): BuddySessionCompositionServices {
  return {
    ...
    createCapabilities: createBuddyCapabilityFactory(resolveBuddyPlatform('linux'), {
      ...
      peer: { request: async () => { throw new Error('No host action is expected during composition') } },
    }),
    directoryGrants: {
  ...
  } as unknown as BuddySessionCompositionServices
}
