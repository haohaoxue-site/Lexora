  ...
  return {
    ...
    systemPrompt: [
      ...
      [
        ...
        'Use supplied native content directly when the task requires understanding the sent snapshot; it is already present in this request, not waiting to be opened or played. For native audio, listen directly; transcribe or describe it when requested. Do not call read, run playback, or run separate transcription merely to understand audio already supplied. To inspect a modified working copy or source file, obtain its current content with an appropriate tool; the supplied native snapshot does not represent those edits. read returns text or supported images; it does not play audio or video or extract PDF text.',
