...
export function createBrowserToolPresentation(
  ...
  return {
    ...
    card: 'browser',
    description: null,
    documentRevision: operation === 'observe'
      ? readNonNegativeInteger(details, 'documentRevision')
      : null,
    elementCount: operation === 'observe'
    ...
    observationTruncated: operation === 'observe'
...
function readOperation(toolName: string): BrowserToolOperation | null {
  ...
  if (toolName === BROWSER_OPEN_TOOL_NAME)
    return 'open'
  return toolName === BROWSER_OBSERVE_TOOL_NAME ? 'observe' : null
}
...
function readBrowserEventUrl(rawUrl: string | null): {
  origin: string
  pathname: '/' | '/[redacted]' | '/preview/[redacted]'
} | null {
