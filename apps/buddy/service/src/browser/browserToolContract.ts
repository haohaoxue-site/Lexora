...
export const BROWSER_OBSERVE_TOOL_NAME = 'lexora_browser_observe'
...
export const browserObserveToolParameters = Type.Object({
...
const browserWaitCondition = Type.Union(
  BROWSER_WAIT_CONDITIONS.map(condition => Type.Literal(condition)),
)
...
export const browserActToolParameters = Type.Union([
  ...
  targetlessBrowserAction({
    ...
    timeoutMs: Type.Integer({ maximum: BROWSER_MAX_WAIT_TIMEOUT_MS, minimum: 1 }),
  }),
])
...
export type BrowserObserveToolInput = Static<typeof browserObserveToolParameters>
export type BrowserToolOperation = 'act' | 'observe' | 'open'
...
export function isBrowserObserveToolInput(input: unknown): input is BrowserObserveToolInput {
  return Check(browserObserveToolParameters, input)
}
...
export function classifyBrowserTool(
  ...
  if (event.toolName === BROWSER_OBSERVE_TOOL_NAME) {
    return isBrowserObserveToolInput(event.input)
