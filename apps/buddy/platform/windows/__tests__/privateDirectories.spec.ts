...
vi.mock('../../native/nativeCommand', () => ({ runNativeCommand: vi.fn() }))

const paths = ['C:\\Users\\fixture\\.lexora']
const executable = 'C:\\Lexora\\private-directories.exe'

describe('private directory failure diagnostics', () => {
  it.each([
    ...
    vi.mocked(runNativeCommand).mockResolvedValue({ code: 1, stdout: Buffer.alloc(0), stderr: JSON.stringify(native) })
    const error = await ensureWindowsPrivateDirectories(paths, executable).catch(error => error)
