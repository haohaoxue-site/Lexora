...
const cdpDomSnapshotSchema = z.object({
  documents: z.array(z.object({
    ...
    nodes: z.object({
      attributes: z.array(z.array(z.number().int().nonnegative())).optional(),
...
const BROWSER_TARGET_ACTIONABILITY_FUNCTION = `function () {
  const element = this
  const emptyFieldMetadata = { ariaLabel: '', autocomplete: '', id: '', label: '', name: '', placeholder: '', type: '' }
  if (!(element instanceof Element) || !element.isConnected) {
    return { connected: false, disabled: true, editable: false, fieldMetadata: emptyFieldMetadata, focusable: false, readOnly: true, selectable: false, visible: false }
  }
  ...
  return {
    ...
    visible: Boolean(style && style.display !== 'none' && style.visibility !== 'hidden' && Number(style.opacity) !== 0 && rect.width > 0 && rect.height > 0),
  }
}`
...
export class SemanticBrowserDriverError extends Error {
  ...
  constructor(code: SemanticBrowserDriverErrorCode = 'BROWSER_PAGE_FAILED') {
...
export class SemanticBrowserDriver {
  ...
  async observe(input: SemanticBrowserObservationInput): Promise<BrowserObservation> {
    try {
      ...
      const accessibilityResults = await Promise.all(observedFrameIds.map(async (frameId) => {
  ...
  validateAction(input: SemanticBrowserActionInput): SemanticBrowserTarget | null {
    ...
    if (target.frameId !== input.frameId)
      throw new SemanticBrowserDriverError('BROWSER_TARGET_STALE')
    if (target.inputMode === 'human' && input.action.kind !== 'scroll')
      throw new SemanticBrowserDriverError('BROWSER_HUMAN_INPUT_REQUIRED')
  ...
  async #withLiveTarget(
    ...
    try {
      ...
      try {
        actionability = await this.#readActionability(objectId)
      }
      catch {
        throw new SemanticBrowserDriverError('BROWSER_TARGET_STALE')
      }
      if (!isLiveTargetActionable(action, actionability))
        throw new SemanticBrowserDriverError('BROWSER_TARGET_STALE')
  ...
  async #readActionability(objectId: string) {
...
function isLiveTargetActionable(
  action: BrowserAction,
  ...
  switch (action.kind) {
    case 'click':
      return target.visible && !target.disabled
    case 'fill':
    case 'type':
      return target.visible
        ...
        && target.editable
    case 'press':
      return target.visible && !target.disabled && target.focusable
    case 'select':
      return target.visible && !target.disabled && target.selectable
    case 'scroll':
      return true
    default:
      return false
