...
const FINAL_RESPONSE_REPAIR_INSTRUCTION = [
  ...
  '现在必须基于已有工具结果给用户写自然语言总结。不要再输出任何工具调用协议。',
  '[Lexora 最终回答修复结束]',
].join('\n')

const FINAL_RESPONSE_FALLBACK_TEXT = '工具已经执行完毕，但模型未生成可展示的最终回答。'
...
function shouldRepairFinalResponse(result: StreamingModelCallResult): boolean {
  return result.suppressedToolCallBlockCount > 0 || result.toolCalls.length > 0
}
